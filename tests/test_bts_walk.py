...
def _snapshot(source: bytes, path: str = "src/pkg/mod.py") -> DonorSnapshot:
    root = Path(tempfile.mkdtemp(prefix="leitir-bts-test-"))
    target = root / "donor"
    ...
    return DonorSnapshot(
        ...
        TREE_HASH_ALGORITHM, FULL, root, target,
    )


def _fixture() -> tuple[DonorSnapshot, NodeId, Graph]:
...
def test_budget_exhaustion_is_partial_without_bts(change: dict[str, int]) -> None:
    snapshot, seed, graph = _fixture()
    result = compute_bts(snapshot, seed, graph, _budget(**change), _policy())
...
def test_reachable_unresolved_reject_dominates_budget() -> None:
    ...
    result = compute_bts(snapshot, seed, graph, _budget(max_nodes=1), _policy())
    assert result.status is BTSStatus.REJECT
    assert result.bts is None
    assert result.report.unresolved == (unresolved,)


def _one_edge(target: NodeId) -> tuple[DonorSnapshot, NodeId, Graph]:
    source = b"def seed():\n"
    snapshot = _snapshot(source)
    source_ref = SourceRef("owner/repo", "a" * 40, "src/pkg/mod.py", GitHubTreeSource.git_blob_sha(source), 1, 0, 1, 11)
...
def test_disposition_precedence_exact_then_defaults_then_reject() -> None:
    ...
    snapshot, seed, graph = _one_edge(external)
    explicit = DispositionRule(
        "external-v1", external, BTSDisposition.EXTERNAL,
        ExternalRule(external, "vendor>=1"), "sha256:" + "4" * 64,
    )
    assert compute_bts(snapshot, seed, graph, _budget(), _policy(explicit)).status is BTSStatus.COMPLETE
    assert compute_bts(snapshot, seed, graph, _budget(), _policy()).status is BTSStatus.REJECT
    ...
    assert compute_bts(snapshot, seed, graph, _budget(), _policy(reject)).status is BTSStatus.REJECT

    stdlib = NodeId(NodeOrigin.STDLIB, NodeKind.FUNCTION, "math", "math.sqrt", "stdlib:math:sqrt")
    snapshot, seed, graph = _one_edge(stdlib)
    pure = StdlibInterface("math", "math.sqrt")
    assert compute_bts(snapshot, seed, graph, _budget(), _policy(stdlib=(pure,))).status is BTSStatus.COMPLETE
...
def test_digest_reproduction_and_invalidation() -> None:
    ...
    assert compute_bts(snapshot, seed, graph, _budget(), _policy()).bts == first.bts
    changed_budget = compute_bts(snapshot, seed, graph, _budget(max_edges=101), _policy())
    changed_policy = compute_bts(snapshot, seed, graph, _budget(), replace(_policy(), policy_id="other"))
    ...
    with pytest.raises(BTSError) as caught:
        compute_bts(snapshot, seed, changed_graph, _budget(), _policy())
