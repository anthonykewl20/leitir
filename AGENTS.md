...
- Pull available work with: `gh issue list --label ready-for-agent --state open`. **The ready-for-agent pool is currently empty**: all Search v2 slices (#86–#95) have shipped to `main` and been closed, so neither the v0.1.4 / Search v2 milestone (https://github.com/anthonykewl20/leitir/milestone/4) nor any other milestone currently has open ready-for-agent issues. The v0.1.2 / BTS milestone (https://github.com/anthonykewl20/leitir/milestone/2) remains open; its exit gate is #73 (generalize the empty-project acceptance gate to N donors — owner-scoped external nsjail artifacts), so BTS is not blocked on agent work.
...
- Target: 1887+ passed (current), 0 failed.
