...
AST or semantic matching. (Wide path: `src/leitir/discovery_search.py:40-42`,
`src/leitir/discovery_search.py:221-227`, `src/leitir/discovery_search.py:437-450`;
...
`src/leitir/discovery_search.py:453-457`, `src/leitir/discovery_search.py:588-605`.)

### Endpoint, credentials, and redirects

The production transport targets `https://api.github.com/search/code` and
limits `per_page` to 100 when constructing the request. (`src/leitir/discovery_search.py:40-42`,
`src/leitir/discovery_search.py:221-227`.) It sends GitHub's JSON media type,
...
HTTPS on `api.github.com`. (`src/leitir/discovery_search.py:202-219`.)
...
through `max_pages`. (`src/leitir/discovery_search.py:431-450`,
`src/leitir/discovery_search.py:453-475`.)
...
These conditions are implemented at `src/leitir/discovery_search.py:476-497`.
...
(`src/leitir/discovery_search.py:481-483`.)

Candidate verification attempts are also bounded by `max_results`, including
promoted representatives after a verification failure. (`src/leitir/discovery_search.py:510-525`.)
...
locally. (`src/leitir/discovery_search.py:339-392`.)
...
(`src/leitir/discovery_search.py:394-420`.)
...
of the HTML `/blob/<sha>/` URL. (`src/leitir/discovery_search.py:51-65`,
`src/leitir/discovery_search.py:241-273`.) Leitir then fetches bytes at the
...
(`src/leitir/discovery_search.py:123-155`, `src/leitir/discovery_search.py:499-525`.)
...
candidate blobs. (`src/leitir/discovery_search.py:481-483`,
`src/leitir/discovery_search.py:588-595`.) The correct verdict is therefore
...
Global dispositions are defined at `src/leitir/discovery_search.py:339-420`.
Scoped matching behavior is implemented at `src/leitir/engine.py:35-47`,
`src/leitir/engine.py:180-197`, and `src/leitir/adapters.py:123-196`.
...
`src/leitir/adapters.py:348-349`, `src/leitir/engine.py:241-245`.)
...
- It uses the legacy REST `/search/code` endpoint, not a local index or the
  newer GitHub.com code-search experience. (`src/leitir/discovery_search.py:174-227`.)
- Leitir's defaults are 30 results and 10 pages, with no CLI flags currently
  exposing those constructor settings. (`src/leitir/discovery_search.py:431-450`,
...
- Wide coverage is always `INDETERMINATE_GLOBAL`; `total_count` is not an
  exhaustive local denominator. (`src/leitir/discovery_search.py:481-483`,
  `src/leitir/discovery_search.py:588-595`.)
...
- Retry protects against transient transport failures, but it cannot remove
  ...
  (`src/leitir/_http.py:262-315`, `src/leitir/discovery_search.py:481-497`.)
...
|---|---|---:|---|---|
| Expose `--max-results`, `--max-pages`, and `--language` | Makes existing internal controls and language intent visible to callers; no new search semantics required. (`src/leitir/discovery_search.py:437-450`, `src/leitir/cli.py:341-364`) | S | Low | Yes |
| Investigate migration to newer GitHub Code Search | The current transport is REST `/search/code`; a migration could improve query capability, but endpoint semantics, auth, pagination, and provenance need research against the current GitHub product/API contract. (`src/leitir/discovery_search.py:174-227`) | M-L | Medium | Possible: GraphQL/HTTP can use `urllib`, but API support must be verified first |
