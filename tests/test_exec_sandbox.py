...
def _policy(fake_nsjail: tuple[Path, str, str], *, output_limit: int = 4096, wall_time: int = 2) -> ContainmentPolicy:
    ...
    mount = ReadOnlyMount("/", "/pinned/rootfs", root_digest)
    mount_payload = {
        "readonly_mounts": [{"destination": "/", "source": "/pinned/rootfs", "source_digest": root_digest}],
...
from leitir.exec_sandbox import *
root = 'sha256:' + '1' * 64
mounts = (ReadOnlyMount('/', '/pinned/rootfs', root),)
payload = {{'readonly_mounts':[{{'destination':'/','source':'/pinned/rootfs','source_digest':root}}], 'rootfs_digest':root, 'writable_tmpfs':'/work', 'writable_tmpfs_bytes':1048576, 'writable_tmpfs_inodes':128}}
import hashlib
md = 'sha256:' + hashlib.sha256((json.dumps(payload, sort_keys=True, separators=(',', ':')) + '\\n').encode()).hexdigest()
p = ContainmentPolicy(POLICY_SCHEMA, {str(path)!r}, {_digest(path.read_bytes())!r}, {version!r}, {build_identity!r}, 'sha256:'+'2'*64, platform.machine(), root, md, mounts, '/work', 1048576, 128, '/work', 'ONCE', False, True, True, True, True, True, True, False, 67108864, 16, 500, 'DEFAULT KILL {{ read, write, exit, exit_group }}', 2, 64, 1, 1, 32, 16, 8, 0, 4096, ('LANG=C.UTF-8','PYTHONHASHSEED=0','TZ=UTC'), True)
