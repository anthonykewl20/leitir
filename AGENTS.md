...
- Pull available work with: `gh issue list --label ready-for-agent --state open`. **The ready-for-agent pool is currently empty; promotion is owner-gated.** PRs #163–#179 are landed: they cover contained execution, security P2 remediation, contained benchmark baselines, the published rootfs, and the held ratification ceremony. #42, #75, and #148 have complete evidence with tracker closure pending; #73 and #52 remain open because `ratified_runtime_digest` is deliberately null while staged mount-source tree digests drift. The next owner step is entry-level instrumentation, then re-sign after a stable digest.
