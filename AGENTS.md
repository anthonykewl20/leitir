...
- The 2026-08-14 live-benchmark hardening added verified local-shelf search, Go API surface and license detection, and `go-module-zip` with SumDB authentication per ADR-0020 (issues #143–#148). SumDB reads are bounded to 1 MiB and fail closed with `SumDBVerificationError`. #144 is resolved by retaining the full tree-hash eligibility requirement for trigram indexes; verified local-byte shelves use the documented local-shelf exception.
...
- Target: 2408 passed / 113 skipped (current), 0 failed; installing the tree-sitter extra runs 105 additional polyglot tests.
